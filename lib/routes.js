...
// fixme: use default args for data, passd in or def.initial?
function createBoard(type) {
    const definition = boardTypes[type];
    return {
        id: shortId.generate(),
        type: type,
        styles: '',
...
module.exports = function(app, config) {
    ...
    app.get('/example/:type', (req, res) => {
        const view = `boards/${ req.params.type }`;
        const board = examples[req.params.type];
        board.includeDefaultStyles = true;
        res.render(view, {
            board: board,
            data: board.data
        });
